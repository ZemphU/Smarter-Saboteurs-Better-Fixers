...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("experiment_arcc.log"),
...
async def run_experiment_loop():
    ...
    results_file = os.path.join(results_dir, 'qwen_results_arcc.jsonl')
    traces_file = os.path.join(results_dir, 'qwen_traces_arcc.jsonl')
    ...
    for model in models:
        ...
        for task in selected_tasks:
            ...
            # Cache for malicious profiles - per task/model combination? 
            # Or should it persist across tasks for the same model?
            # The original logic reset it per model per task.
            ...
            for scenario in scenarios:
               ...
               # Check cache for malicious profile
                ...
                roles_config_coding = experiment_config.get_standard_roles_config(scenario, engineer_profile)
                
                try:
                    outputs, roles = await run_linear_collaboration(task_prompt_coding, roles_config_coding, model_name)
                    
                    # We want the Engineer's output for evaluation
